...
export const Route = createFileRoute("/arcade")({
  validateSearch: (search: Record<string, unknown>) => ({
    game: typeof search.game === "string" ? search.game : "taqha",
...
function ArcadePage() {
  const { game: requestedGame, room } = Route.useSearch();
  const game = arcadeGame(requestedGame);
  return <ArcadeSession key={game.slug} game={game} initialRoomCode={room} />;
}

/** A game card always opens a fresh room; old browser saves must never skip setup. */
function ArcadeSession({ game, initialRoomCode }: { game: ArcadeGame; initialRoomCode?: string }) {
  ...
  useEffect(() => {
    try {
      window.localStorage.removeItem(`qad-altahadi:v2:arcade:${game.slug}`);
    } catch {
      /* Storage is optional. */
    }
  }, [game.slug]);
  ...
  return (
    <div className={`min-h-screen arcade-page arcade-${game.accent}`} data-game-stage={stage}>
      <SiteHeader />
      <main
        className={`heritage-page-shell min-h-[60vh] sm:min-h-[76vh] px-3.5 sm:px-4 py-5 sm:py-12 ${game.slug === "huroof" && stage === "play" ? "huroof-fullscreen-page" : ""}`}
      >
        <div
          className={`mx-auto mb-4 sm:mb-8 flex max-w-6xl items-center justify-between gap-3 ${game.slug === "huroof" && stage === "play" ? "huroof-play-nav" : ""}`}
        >
          <Link
            to="/games"
            className="inline-flex items-center gap-1 text-sm font-bold text-muted-foreground transition-colors hover:text-gold"
          >
            <ChevronLeft className="h-4 w-4" /> كل الألعاب
          </Link>
        ...
        {loading ? (
          ...
          <section className="arcade-login mx-auto max-w-xl rounded-[2rem] p-6 sm:p-8 text-center">
            <Crown className="mx-auto h-10 w-10 text-gold" />
            <h1 className="mt-4 text-3xl sm:text-4xl font-black">جاهزين للّمة؟</h1>
            <p className="mt-3 text-sm text-muted-foreground">
              سجّل حسابك أول، وبعدها افتح غرفة وخلّوا اللعب يبلّش.
            </p>
            <Button asChild className="mt-6">
              <Link to="/auth">سجّل دخولك</Link>
            </Button>
          </section>
        ) : stage === "room" ? (
          <GameRoomFlow game={game} initialCode={initialRoomCode} onStart={startRoom} />
        ) : stage === "setup" ? (
          <GameSetup
            game={game.slug}
            mode={mode}
...
function ChoiceButton({
  ...
  return (
    <button onClick={onClick} className={`arcade-choice ${active ? "is-active" : ""}`}>
      {children}
    </button>
  );
}
...
function GameSetup({
  ...
  if (game === "outsider")
    return <OutsiderSetup mode={mode} initialPlayers={initialPlayers} onStart={onStart} />;
  if (game === "auction" || game === "auction-billion")
    return (
      <AuctionSetup
        mode={mode}
        initialKind={game === "auction-billion" ? "billion" : "categories"}
        onStart={onStart}
      />
    );
...
function HuroofSetup({
  ...
  return (
    <section className="arcade-panel arcade-huroof mx-auto max-w-3xl rounded-[2rem] p-6 sm:p-9">
      <span className="eyebrow">حروف · {mode === "online" ? "غرفة جوالات" : "جهاز واحد"}</span>
      <h1 className="mt-3 text-4xl">شبكة سيطرة، مش تكوين كلمات</h1>
      <p className="mt-2 text-muted-foreground">
        جاوبوا سؤال، اختاروا خلية، ووصلوا خط بلون فريقكم: الأخضر من فوق لتحت والعنابي من اليمين
        للشمال.
      </p>
      ...
      <div className="mt-6 grid gap-6 sm:grid-cols-2">
        ...
        <SetupChoices
          ...
          values={[4, 5, 6]}
          selected={size}
          onChange={(value) => setSize(value as 4 | 5 | 6)}
        />
      </div>
      <div className="mt-5 flex flex-wrap gap-2">
        <ChoiceButton active={!buzzer} onClick={() => setBuzzer(false)}>
          وضع المقدم
        </ChoiceButton>
        <ChoiceButton active={buzzer} onClick={() => setBuzzer(true)}>
          وضع الجرس
        </ChoiceButton>
      </div>
      <div className="huroof-rules mt-7">
        <Check className="h-5 w-5" />
        <p>
          <b>مدير الجلسة</b> بقرأ السؤال للفريق. الجواب الصح بخليه يملك الخلية ويكمل دوره؛ الغلط
          بمرّر الدور للفريق الثاني.
        </p>
      </div>
      <Button
        className="mt-8"
        onClick={() =>
          onStart({
            teams: [teamA.trim() || "فريق السرو", teamB.trim() || "فريق الكرمل"],
            players: [],
            huroof: { size, rounds, buzzer, seed: Math.random() },
          })
        }
      >
        <Play /> بلّشوا حروف
  ...
  return (
    <div>
      ...
      <div className="flex gap-2">
        {values.map((value) => (
          <ChoiceButton key={value} active={value === selected} onClick={() => onChange(value)}>
            {value}
          </ChoiceButton>
        ))}
      </div>
...
function OutsiderSetup({
  ...
  const [names, setNames] = useState(
    initialPlayers.length >= 5 ? initialPlayers : ["ليان", "يزن", "تالا", "حمزة", "نور"],
  );
  const [turns, setTurns] = useState(1);
  const [category, setCategory] = useState("عام");
  function setPlayers(value: number) {
    setNames((current) =>
      value > current.length
        ? [
            ...
            ...Array.from(
              ...
              (_, index) => `لاعب ${current.length + index + 1}`,
            ),
          ]
  ...
  return (
    <section className="arcade-panel arcade-outsider mx-auto max-w-3xl rounded-[2rem] p-6 sm:p-9">
      <span className="eyebrow">
        مين برا السالفة · {mode === "online" ? "كل واحد بجواله" : "جهاز واحد"}
      </span>
      <h1 className="mt-3 text-4xl">سرّ واحد... ولاعب برا</h1>
      <p className="mt-2 text-muted-foreground">
        الكل بيعرف الكلمة إلا واحد. انتبهوا مين بحكي كلام عام كثير.
      </p>
      <div className="mt-7 grid gap-6 sm:grid-cols-3">
        <Counter title="عدد اللاعبين" value={names.length} min={5} max={12} onChange={setPlayers} />
        <SetupChoices title="جولات الوصف" values={[1, 2]} selected={turns} onChange={setTurns} />
        <div>
          <p className="mb-2 text-sm font-bold">فئة الكلمات</p>
          <div className="flex flex-wrap gap-2">
            {["عام", "أكل", "أماكن"].map((item) => (
              <ChoiceButton key={item} active={category === item} onClick={() => setCategory(item)}>
                {item}
              </ChoiceButton>
            ))}
          </div>
      ...
      <Button
        ...
        onClick={() =>
          onStart({
            ...EMPTY_SESSION,
            players: names.map((name, index) => name.trim() || `لاعب ${index + 1}`),
          })
        }
      >
        <Eye /> وزّعوا الأسرار
...
function MafiaSetup({
  ...
  const [names, setNames] = useState(
    initialPlayers.length >= 5
      ? initialPlayers
      : ["ليان", "يزن", "تالا", "حمزة", "نور", "آدم", "رنا"],
  );
  const [gameMode, setGameMode] = useState("كلاسيكي");
  function setPlayers(value: number) {
    setNames((current) =>
      value > current.length
        ? [
  ...
  const mafia = Math.max(1, Math.floor(names.length / 4));
  const special = gameMode === "كلاسيكي" ? 2 : gameMode === "كلاسيكي + سفاح" ? 3 : 4;
  return (
    <section className="arcade-panel arcade-mafia mx-auto max-w-3xl rounded-[2rem] p-6 sm:p-9">
      <span className="eyebrow">مافيا · {mode === "online" ? "كل واحد بجواله" : "راوي واحد"}</span>
      <h1 className="mt-3 text-4xl">الليل إله قوانين</h1>
      <p className="mt-2 text-muted-foreground">
        اختاروا النمط، والنظام بيوزع الأدوار ويحسب الفوز تلقائياً.
      </p>
      <div className="mt-7 grid gap-6 sm:grid-cols-[.7fr_1.3fr]">
        <Counter title="عدد اللاعبين" value={names.length} min={5} max={20} onChange={setPlayers} />
        <div>
          <p className="mb-2 text-sm font-bold">نمط اللعبة</p>
          <div className="grid grid-cols-2 gap-2">
            {["كلاسيكي", "كلاسيكي + سفاح", "متقدم", "حرب شاملة", "مخصص"].map((item) => (
              <ChoiceButton key={item} active={gameMode === item} onClick={() => setGameMode(item)}>
                {item}
              </ChoiceButton>
            ))}
          </div>
      ...
      <ParticipantFields names={names} onChange={setNames} />
      <div className="mafia-summary mt-6">
        <Skull />
        <div>
          <small>التوزيع التلقائي</small>
          <strong>
            {mafia} مافيا | {special} أدوار خاصة | {names.length - mafia - special} مواطنين
          </strong>
        </div>
      </div>
      <Button
...
function ParticipantFields({
  ...
  return (
    <div className="participant-fields mt-7">
      <div className="flex items-center justify-between gap-3">
        <p className="font-bold">أسماء المشاركين</p>
        <span>{names.length} لاعبين</span>
      </div>
      <p className="mt-1 text-sm text-muted-foreground">
        اكتبوا الأسماء قبل البدء؛ رح تظهر بالأدوار والتصويت.
      </p>
      <div className="mt-4 grid gap-2 sm:grid-cols-2">
        {names.map((name, index) => (
          <Input
            ...
            onChange={(event) =>
              onChange(
                names.map((item, itemIndex) => (itemIndex === index ? event.target.value : item)),
              )
            }
...
function AuctionSetup({
  ...
  return (
    <section className="arcade-panel arcade-auction mx-auto max-w-3xl rounded-[2rem] p-6 sm:p-9">
      <span className="eyebrow">
        {kind === "billion" ? "مزاد المليار" : "مزاد الأسئلة"} ·{" "}
        {mode === "online" ? "غرفة جوالات" : "جهاز واحد"}
      </span>
      <h1 className="mt-3 text-4xl">
        {kind === "billion" ? "ابنوا فريقكم بالمزايدة" : "زايدوا وثبّتوا كلمتكم"}
      </h1>
      <div className="mt-7 grid gap-4 sm:grid-cols-2">
        <label className="text-sm font-bold">
          {kind === "billion" ? "اسم اللاعب الأول" : "اسم الفريق الأول"}
          <Input
            value={teamA}
            onChange={(event) => setTeamA(event.target.value)}
            className="mt-2 h-11 bg-background/45"
        ...
        <label className="text-sm font-bold">
          {kind === "billion" ? "اسم اللاعب الثاني" : "اسم الفريق الثاني"}
          <Input
            value={teamB}
            onChange={(event) => setTeamB(event.target.value)}
            className="mt-2 h-11 bg-background/45"
      ...
      <div className="mt-6 grid gap-5 sm:grid-cols-3">
        {kind === "billion" ? (
          <>
            <SetupChoices
              title="ميزانية كل لاعب"
              values={[100, 200]}
              selected={budget}
              onChange={setBudget}
            />
            <p className="self-end text-sm leading-7 text-muted-foreground">
              سبع جولات ثابتة: حارس، دفاعان، وسطَان، ومهاجمان. في كل جولة بطاقة علنية وبطاقة خفية من
              نفس المركز.
            </p>
          </>
        ) : (
          <>
            ...
            />
            <p className="self-end text-sm leading-7 text-muted-foreground">
              ما في سقف للمزايدة: اكتبوا أي رقم أعلى من العرض. النقاط تُحسب من الإجابات الصحيحة: كل
              10 إجابات = نقطة.
            </p>
          </>
        )}
      ...
      {kind === "billion" && (
        <div className="mt-6">
          <p className="mb-2 text-sm font-bold">اختاروا اللاعبين المعروضين بالمزاد</p>
          <div className="flex flex-wrap gap-2">
            {[
              { id: "all", label: "كل الأندية" },
              { id: "premier", label: "الدوري الإنجليزي" },
              { id: "laliga", label: "الدوري الإسباني" },
              { id: "seriea", label: "الدوري الإيطالي" },
              { id: "bundesliga", label: "الدوري الألماني" },
              { id: "ligue1", label: "الدوري الفرنسي" },
              { id: "legends", label: "الأساطير" },
            ].map((item) => (
              <ChoiceButton
                key={item.id}
                active={pool === item.id}
          ...
          <p className="mt-2 text-xs text-muted-foreground">
            إذا لم يكفِ المركز المختار لبطاقتين، يضيف النظام بطاقة احتياطية من النجوم لضمان اكتمال
            الجولة.
          </p>
        </div>
      )}
      <p className="mt-4 text-sm text-muted-foreground">
        سمّوا الطرفين أولاً؛ الأسماء تظهر على الملعب ولوحة النقاط طوال اللعبة.
      </p>
      <Button
        className="mt-5"
        ...
        onClick={() =>
          onStart({
            teams: [teamA.trim(), teamB.trim()],
            players: [],
            auction: { kind, rounds, slots: 7, budget, pool },
          })
        }
      >
        <Gavel /> افتحوا المزاد
...
function GamePlay({
  ...
  if (game === "auction-billion")
    return (
      <BillionAuctionGame
        teams={session.teams}
        budget={session.auction?.budget ?? 200}
        pool={session.auction?.pool}
      />
    );
...
const FULL_FORMATION: BillionRole[] = [
  "GK",
  "DEF",
  "DEF",
  "DEF",
...
function formationFor(slots: number) {
  return slots < 11 ? QUICK_FORMATION : FULL_FORMATION;
}
...
function SquadPitch({
  ...
  return (
    <article className={`billion-squad billion-squad-${side}`}>
      <header>
        <div>
          <small>ميزانية متبقية</small>
          <strong>{budget.toLocaleString("en-US")}M</strong>
        </div>
        <h2>{name}</h2>
        <span>
          {squad.length}/{slots} صفقة
        </span>
      </header>
      <div className={`billion-pitch slots-${formation.length}`} aria-label={`ملعب ${name}`}>
        {formation.map((role, index) => {
          ...
          return (
            <div
              className={`pitch-player role-${role.toLowerCase()} ${player ? "is-filled" : ""}`}
              key={`${role}-${index}`}
            >
...
function AuctionPlay({ session }: { session: GameSession }) {
  const config = session.auction ?? {
    kind: "categories" as const,
    rounds: 3,
    slots: 19,
    budget: 200,
  };
  const [phase, setPhase] = useState<"pick" | "bid" | "challenge" | "result" | "twist">("pick");
  const [questionIndex, setQuestionIndex] = useState(0);
  const [markedAnswers, setMarkedAnswers] = useState<string[]>([]);
  const [bid, setBid] = useState(config.kind === "billion" ? BILLION_AUCTION_PLAYERS[0].price : 3);
  ...
  const [budgets, setBudgets] = useState<[number, number]>([
    config.budget ?? 200,
    config.budget ?? 200,
  ]);
  const [squads, setSquads] = useState<[BillionAuctionPlayer[], BillionAuctionPlayer[]]>([[], []]);
  const teams = session.teams;
  const featured = BILLION_AUCTION_PLAYERS[playerIndex];
  const question = AUCTION_QUESTIONS[questionIndex % AUCTION_QUESTIONS.length];
  useEffect(() => {
  ...
  function beginBid() {
    setBid(Math.min(question.suggestedBid, answerLimit));
    setCustomBid("");
    setBidError("");
    setBidder(0);
    setWinner(null);
  ...
  function placeQuestionBid(amount: number) {
    if (!Number.isInteger(amount) || amount <= bid || amount > answerLimit) {
      setBidError(`المزايدة يجب أن تكون أكبر من ${bid} ولا تتجاوز ${answerLimit} إجابة متاحة.`);
      return;
    }
    setBid(amount);
    setCustomBid("");
    setBidError("");
    setBidder((current) => (current === 0 ? 1 : 0));
  }
  function placeBillionBid(amount: number) {
    if (!Number.isInteger(amount) || amount <= bid || amount > budgets[bidder]) {
      setBidError(`العرض لازم يكون أكبر من ${bid} وما يتجاوز رصيد ${teams[bidder]}.`);
      return;
    }
    setBid(amount);
    setCustomBid("");
  ...
  function passBillionBid() {
    ...
    if (budgets[buyer] < bid) {
      setBidError(`رصيد ${teams[buyer]} لا يكفي للصفقة.`);
      return;
    }
    if (squads[buyer].length >= config.slots) {
      setBidError(`${teams[buyer]} أكمل خانات تشكيلته؛ مرّروا الصفقة للطرف الثاني.`);
      return;
    }
    setBudgets((value) => (buyer === 0 ? [value[0] - bid, value[1]] : [value[0], value[1] - bid]));
    setSquads((value) =>
      buyer === 0 ? [[...value[0], featured], value[1]] : [value[0], [...value[1], featured]],
    );
    const nextIndex = playerIndex + 1;
    setPlayerIndex(nextIndex);
    setBid(BILLION_AUCTION_PLAYERS[nextIndex]?.price ?? 0);
    setBidder(bidder);
    setCustomBid("");
    setBidError("");
  }
  function withdraw() {
    const winningTeam = bidder === 0 ? 1 : 0;
  ...
  function nextRound() {
    ...
    setPhase(next % 4 === 0 ? "twist" : "pick");
  }
  if (config.kind === "billion") {
    const completed = squads[0].length >= config.slots && squads[1].length >= config.slots;
    if (!featured || completed)
      return (
        <section className="auction-play auction-billion mx-auto max-w-5xl rounded-[2rem] p-8 text-center">
          <Trophy className="mx-auto h-12 w-12 text-gold" />
          <span className="eyebrow">انتهى المزاد</span>
          <h1 className="mt-4">جاهزة التشكيلات للمقارنة</h1>
          <div className="billion-pitches mt-7">
            <SquadPitch
              name={teams[0]}
              budget={budgets[0]}
              squad={squads[0]}
    ...
    return (
      <section className="auction-play auction-billion mx-auto max-w-[92rem] rounded-[2rem] p-5 text-center sm:p-8">
        <span className="eyebrow">
          مزاد المليار · الصفقة {playerIndex + 1} من {BILLION_AUCTION_PLAYERS.length}
        </span>
        <p className="mt-2 text-sm text-muted-foreground">
          {teams[bidder]} يزايد الآن. مرّر لتذهب الصفقة للطرف الثاني على آخر رقم معلن.
        </p>
        <div className="billion-pitches mt-6">
          ...
          <div className="billion-auction-desk">
            <article className="football-auction-card">
              {featured.asset ? (
                <img src={billionPlayerAsset(featured.asset)} alt={`بطاقة ${featured.name}`} />
              ) : (
                <Gavel />
              )}
              <small>{featured.position}</small>
              <h1>{featured.name}</h1>
              <p>
                التقييم {featured.rating} · يبدأ من {featured.price}M
              </p>
            </article>
            ...
            <div className="auction-bid-controls mt-5">
              <Button disabled={bid + 5 > budgets[bidder]} onClick={() => placeBillionBid(bid + 5)}>
                <Gavel /> +٥M
              </Button>
              <Button
                disabled={bid + 10 > budgets[bidder]}
                onClick={() => placeBillionBid(bid + 10)}
              >
                +١٠M
              </Button>
              <Button
                disabled={bid + 25 > budgets[bidder]}
                onClick={() => placeBillionBid(bid + 25)}
              >
                +٢٥M
              </Button>
              <Input
                ...
                placeholder="اكتب أي رقم بالملايين"
                aria-label="مزايدة مخصصة بالملايين"
              />
              <Button variant="outline" onClick={() => placeBillionBid(Number(customBid))}>
                زايد
              </Button>
              <Button variant="outline" onClick={passBillionBid}>
                أمرّر
              </Button>
            </div>
  ...
  if (phase === "pick")
    return (
      <section className="auction-play mx-auto max-w-4xl rounded-[2rem] p-8 text-center">
        <span className="eyebrow">
          مزاد الأسئلة · الجولة {round} من {config.rounds}
        </span>
        <h1 className="mx-auto mt-6 max-w-3xl text-4xl leading-relaxed">{question.prompt}</h1>
        <p className="mt-5 text-muted-foreground">
          لا تظهر الإجابات الآن؛ كل فريق يزايد فقط على العدد الذي يقدر يذكره.
        </p>
        <Button className="mt-8" onClick={beginBid}>
          <Gavel /> ابدأ المزايدة
        </Button>
      </section>
    );
  if (phase === "bid")
    return (
      <section className="auction-play mx-auto max-w-3xl rounded-[2rem] p-8 text-center">
        ...
        <h1 className="mt-4">الدور على {teams[bidder]}</h1>
        <div className="auction-bid-number">
          {bid}
          <small>إجابة</small>
        </div>
        <p className="mt-3 text-muted-foreground">
          الحد الأقصى لهذا السؤال: {answerLimit} إجابة. زايدوا برقم أعلى من العرض الحالي أو مرّروا
          للفريق الآخر. كل 10 إجابات صحيحة = نقطة.
        </p>
        <div className="auction-bid-controls mt-7">
          <Button disabled={bid + 1 > answerLimit} onClick={() => placeQuestionBid(bid + 1)}>
            +١
          </Button>
          <Button disabled={bid + 2 > answerLimit} onClick={() => placeQuestionBid(bid + 2)}>
            +٢
          </Button>
          <Button disabled={bid + 5 > answerLimit} onClick={() => placeQuestionBid(bid + 5)}>
            +٥
          </Button>
          <Input
            inputMode="numeric"
            ...
            aria-label="مزايدة مخصصة"
          />
          <Button
            variant="outline"
            disabled={bid >= answerLimit}
            onClick={() => placeQuestionBid(Number(customBid))}
          >
            زايد
          </Button>
  ...
  if (phase === "challenge")
    return (
      <section className="auction-play mx-auto max-w-5xl rounded-[2rem] p-8 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          مدير اللعبة يعلّم الإجابة التي يسمعها فقط.
        </p>
        <div className="auction-answer-list mt-7">
          {question.answers.map((answer) => (
            <button
              key={answer}
              className={markedAnswers.includes(answer) ? "is-marked" : ""}
              onClick={() =>
                setMarkedAnswers((current) => {
                  const next = current.includes(answer)
                    ? current.filter((item) => item !== answer)
                    : [...current, answer];
                  setCorrect(next.length);
                  return next;
                })
              }
            >
        ...
        <div className="mt-6 flex justify-center gap-3">
          <Button onClick={finishChallenge} disabled={correct < bid && seconds > 0}>
            ثبّت النتيجة
          </Button>
          {seconds === 0 && (
            <Button variant="outline" onClick={finishChallenge}>
              انتهى الوقت
            </Button>
          )}
        </div>
  ...
  if (phase === "twist") {
    const twist = AUCTION_TWISTS[(round / 4 - 1) % AUCTION_TWISTS.length];
    return (
      <section className="auction-play auction-twist mx-auto max-w-xl rounded-[2rem] p-9 text-center">
        <Gavel className="auction-gavel mx-auto" />
        <span className="eyebrow">عقوبة مفاجئة</span>
        <div className="twist-card mt-6">
          <small>اقلبوا الكرت</small>
          <strong>{twist}</strong>
        </div>
        <Button className="mt-7" onClick={() => setPhase("pick")}>
          السؤال التالي
        </Button>
      </section>
    );
    ...
    <section className="auction-play mx-auto max-w-xl rounded-[2rem] p-8 text-center">
      ...
      <p className="mt-3 text-lg">
        {correct >= bid
          ...
          : `${teams[(winner ?? 0) === 0 ? 1 : 0]} أخذ الهدية.`}
      </p>
      <p className="mt-2 font-bold text-gold">
      ...
      <div className="auction-scoreboard">
        <strong>
          {teams[0]} <b>{scores[0]}</b>
        </strong>
        <strong>
          {teams[1]} <b>{scores[1]}</b>
        </strong>
      </div>
      {round < config.rounds ? (
        <Button className="mt-7" onClick={nextRound}>
          الجولة التالية
        </Button>
      ) : (
        <Button asChild className="mt-7">
          <Link to="/games">لعبة جديدة</Link>
        </Button>
      )}
    </section>
  );
...
const HUROOF_SVG_LAYOUTS = {
  ...
  6: { width: 576, height: 489 },
} as const;

function SuppliedHuroofGrid({
  size,
  letters,
  owners,
  selected,
  ...
  const centerFor = (row: number, column: number) => ({
    x: 44 + (row % 2 ? 44 : 0) + column * 88.5,
    y: 53 + row * 77,
  });
  const hexPoints = (x: number, y: number) =>
    `${x},${y - cellHeight / 2} ${x + cellWidth / 2},${y - 25} ${x + cellWidth / 2},${y + 24} ${x},${y + cellHeight / 2} ${x - cellWidth / 2},${y + 24} ${x - cellWidth / 2},${y - 25}`;
  return (
    <svg
      className="huroof-supplied-grid"
      viewBox={`0 0 ${layout.width} ${layout.height}`}
      role="grid"
      aria-label={`شبكة حروف ${size} في ${size}`}
    >
      {letters.map((_, index) => {
        ...
        return (
          <polygon
            key={`base-${index}`}
            className={`huroof-svg-cell-bg ${owner ? `team-${owner}` : ""} ${selected === index ? "is-selected" : ""}`}
            points={hexPoints(x, y)}
          />
        );
      })}
      <image
        className="huroof-svg-art"
        href={`/assets/huroof/mask-group-${size}.svg`}
        width={layout.width}
      ...
      {letters.map((letter, index) => {
        ...
        return (
          <g
            key={`${letter}-${index}`}
            className={`huroof-svg-cell ${owner ? `team-${owner}` : ""} ${selected === index ? "is-selected" : ""} ${winningPath.includes(index) ? "is-winning" : ""}`}
            role="gridcell"
...
function HuroofPlay({ session }: { session: GameSession }) {
  ...
  function undo() {
    ...
    if (!previous) return;
    setOwners(previous.owners);
    setTurn(previous.turn);
    setHistory((current) => current.slice(0, -1));
    setWinningPath(
      findWinningPath(previous.owners, config.size, "A").length
        ? findWinningPath(previous.owners, config.size, "A")
        : findWinningPath(previous.owners, config.size, "B"),
    );
  }
  useEffect(() => {
    try {
      ...
      if (saved) {
        const parsed = JSON.parse(saved) as Partial<{
          ...
          turn: "A" | "B";
          winningPath: number[];
          history: HuroofSnapshot[];
        }>;
        if (parsed.owners?.length === config.size * config.size) {
          setOwners(parsed.owners);
          setTurn(parsed.turn ?? "A");
          setWinningPath(parsed.winningPath ?? []);
  ...
  useEffect(() => {
    ...
    try {
      window.localStorage.setItem(
        `taqha:huroof:${config.seed}`,
        JSON.stringify({ owners, turn, winningPath, history }),
      );
    } catch {
      /* Keep the round playable if storage is unavailable. */
    }
  ...
  return (
    <section className="huroof-stage mx-auto w-full max-w-none">
      <header className="huroof-topbar">
        <ScoreHex team="A" name={teamA} score="0" active={turn === "A"} />
        <div>
          <span>حروف</span>
          <strong>الجولة 1 من {config.rounds}</strong>
          <small>{config.buzzer ? "وضع الجرس" : "وضع المقدم"}</small>
        </div>
        <ScoreHex team="B" name={teamB} score="0" active={turn === "B"} />
      </header>
      ...
      {winner ? (
        <div className="huroof-win-panel">
          <Crown />
          <h1>فريق {winner === "A" ? teamA : teamB} ربط الطرفين!</h1>
          <p>وصلتوها — انتهت الجولة بمسار متصل.</p>
          <div>
            <Button
              onClick={() => {
                setOwners(Array(config.size * config.size).fill(null));
                setWinningPath([]);
                setHistory([]);
                setTurn("A");
              }}
        ...
        <footer className="huroof-actionbar">
          <span>
            الدور على <b>{turn === "A" ? teamA : teamB}</b> — اختروا خلية لبدء السؤال
          </span>
          <Button variant="outline" disabled={!history.length} onClick={undo}>
            تراجع عن آخر حركة
          </Button>
        </footer>
      ) : (
        <section className="huroof-question-stage">
          ...
          <div className="huroof-question-actions">
            ...
            <Button onClick={() => claim("A")}>
              ✓ الأخضر صح <small>A</small>
            </Button>
            <Button className="bg-[#7e1f35] hover:bg-[#6d192e]" onClick={() => claim("B")}>
              ✓ العنابي صح <small>L</small>
            </Button>
            <Button variant="outline" onClick={miss}>
              لا أحد أجاب
            </Button>
          </div>
          <p>اختصارات: A للأخضر · L للعنابي · Space للإجابة</p>
        </section>
      )}
    </section>
  );
...
function ScoreHex({
  ...
  return (
    <div className={`huroof-score team-${team.toLowerCase()} ${active ? "is-active" : ""}`}>
      <small>{name}</small>
      <strong>{score}</strong>
    </div>
  );
...
function OutsiderPlay({ mode, names: sessionNames }: { mode: PlayMode; names: string[] }) {
  ...
  const [phase, setPhase] = useState<"secret" | "describe" | "vote" | "result">("secret");
  const [outsider, setOutsider] = useState(0);
  const [voter, setVoter] = useState(0);
  const [voteRevealed, setVoteRevealed] = useState(false);
  const [votes, setVotes] = useState<number[]>([]);
  useEffect(() => {
    setOutsider(Math.floor(Math.random() * names.length));
  }, [names.length]);
  if (phase === "secret")
    return (
      <section className="outsider-play mx-auto max-w-xl rounded-[2rem] p-7 text-center">
        <span className="eyebrow">
          بطاقة سرية · {step + 1} من {names.length}
        </span>
        <h1 className="mt-4 text-4xl">{names[step]}</h1>
        <div className={`secret-card mt-6 ${revealed ? "is-open" : ""}`}>
          {revealed ? (
            step === outsider ? (
              <>
                <Search />
                <strong>إنت برا السالفة!</strong>
                <p>راقب الوصف وحاول تلتقط الكلمة.</p>
              </>
            ) : (
              <>
                <CircleHelp />
                <small>كلمتكم السرية</small>
                <strong>مطعم</strong>
              </>
            )
          ) : (
            <>
              <Eye />
              <strong>خد الجهاز لحالك</strong>
              <p>تأكد ما حدا شايف الشاشة قبل ما تكشف بطاقتك.</p>
            </>
          )}
        </div>
        <Button
          ...
          onClick={() =>
            revealed
              ? step === names.length - 1
                ? (setStep(0), setPhase("describe"))
                : (setStep(step + 1), setRevealed(false))
              : setRevealed(true)
          }
        >
          {revealed
            ? step === names.length - 1
              ? "اخفوا البطاقات وبلّشوا الوصف"
              : "اخفِ البطاقة وسلّم الجهاز"
            : "أنا لحالي · اكشف"}
        </Button>
      </section>
    );
  if (phase === "describe")
    return (
      <section className="outsider-play mx-auto max-w-xl rounded-[2rem] p-7 text-center">
        <span className="eyebrow">
          جولة الوصف · {step + 1} من {names.length}
        </span>
        <h1 className="mt-5 text-4xl">دور {names[step]}</h1>
        <p className="mt-3 text-muted-foreground">
          احكي كلمة أو جملة قصيرة، بدون ما تقول الكلمة السرية.
        </p>
        <div className="turn-timer mt-7">00:30</div>
        <Button
          className="mt-6"
          onClick={() =>
            step === names.length - 1
              ? (setVoter(0), setVoteRevealed(false), setPhase("vote"))
              : setStep(step + 1)
          }
        >
          خلصت · التالي
  ...
  if (phase === "vote")
    return (
      <section className="outsider-play mx-auto max-w-xl rounded-[2rem] p-7 text-center">
        <Vote className="mx-auto h-9 w-9 text-gold" />
        <span className="eyebrow mt-3">
          تصويت سري · {voter + 1} من {names.length}
        </span>
        <h1 className="mt-3 text-4xl">{names[voter]}</h1>
        {!voteRevealed ? (
          <>
            <p className="mt-3 text-muted-foreground">خد الجهاز لحالك. ما حدا غيرك يشوف اختيارك.</p>
            <Button className="mt-6" onClick={() => setVoteRevealed(true)}>
              أنا لحالي · افتح التصويت
            </Button>
          </>
        ) : (
          <>
            <p className="mt-3 text-muted-foreground">مين شايفه برا السالفة؟</p>
              ...
              {names.map((name, index) =>
                index === voter ? null : (
                  <Button
                    ...
                    variant="outline"
                    onClick={() => {
                      const nextVotes = [...votes, index];
                      setVotes(nextVotes);
                      if (voter === names.length - 1) setPhase("result");
                      else {
                        setVoter(voter + 1);
  ...
  const tally = votes.reduce<Record<number, number>>(
    (result, choice) => ({ ...result, [choice]: (result[choice] ?? 0) + 1 }),
    {},
  );
  const accused = Number(Object.entries(tally).sort((a, b) => b[1] - a[1])[0]?.[0] ?? -1);
  const caught = accused === outsider;
  return (
    <section className="outsider-result mx-auto max-w-xl rounded-[2rem] p-8 text-center">
      <Sparkles className="mx-auto h-10 w-10 text-gold" />
      <p className="eyebrow mt-3">كشف الحقيقة</p>
      <h1 className="mt-2 text-5xl">البرا كان {names[outsider]}</h1>
      <p className="mt-4 text-lg text-muted-foreground">
        {caught
          ? "كشفتموها! أغلب القعدة عرفت مين كان يلف ويدور."
          : `التصويت راح على ${names[accused] ?? "ما حدا"} — البرا نجح يخبّي حاله.`}
      </p>
      <Button asChild className="mt-7">
        <Link to="/games">لعبة جديدة</Link>
      </Button>
...
function MafiaPlay({ mode, names: sessionNames }: { mode: PlayMode; names: string[] }) {
  const names = sessionNames.length ? sessionNames : ["ليان", "يزن", "تالا", "حمزة", "نور"];
  const [phase, setPhase] = useState<"role" | "night" | "day" | "vote" | "result">("role");
  ...
  const roles = [
    ...Array.from({ length: mafiaCount }, () => "مافيا"),
    "طبيب",
    "محقق",
    ...Array.from({ length: Math.max(0, names.length - mafiaCount - 2) }, () => "مواطن"),
  ];
  const role = roles[roleStep];
  if (phase === "role")
    return (
      <section className="mafia-play mx-auto max-w-xl rounded-[2rem] p-8 text-center">
        <p className="eyebrow">
          بطاقة سرية {roleStep + 1} من {names.length} ·{" "}
          {mode === "online" ? "يفتحها صاحب الجوال فقط" : "سلّم الجهاز لصاحب الدور"}
        </p>
        <h1 className="mt-3 text-3xl">{names[roleStep]}</h1>
        {roleRevealed ? (
          <div className="mafia-role-card mt-6">
            <Skull />
            <small>دورك الليلة</small>
            <h1>{role}</h1>
            <p>
              {role === "محقق"
                ? "اختار لاعب واحد واعرف إذا هو من المافيا."
                : role === "طبيب"
                  ? "اختار لاعب تحميه من ضربة المافيا."
                  : role === "مافيا"
                    ? "اتفقوا بسرّية على اللاعب المستهدف."
                    : "ناقش بالنهار وصوّت لكشف المافيا."}
            </p>
          </div>
        ) : (
          <div className="secret-card mt-6">
            <Eye />
            <strong>خذ الجهاز لحالك</strong>
            <p>تأكد أن القعدة ما بتشوف الشاشة، بعدها اكشف بطاقتك.</p>
          </div>
        )}
        <Button
          className="mt-7"
          onClick={() =>
            !roleRevealed
              ...
              : roleStep === names.length - 1
                ? setPhase("night")
                : (setRoleStep(roleStep + 1), setRoleRevealed(false))
          }
        >
          {!roleRevealed
            ? "أنا لحالي · اكشف دوري"
            : roleStep === names.length - 1
              ? "اخفوا البطاقات وبلّشوا الليل"
              : "اخفِ البطاقة وسلّم الجهاز"}
        </Button>
      </section>
    );
  if (phase === "night")
    return (
      <section className="mafia-play mx-auto max-w-xl rounded-[2rem] p-8 text-center">
        <span className="eyebrow">مرحلة الليل</span>
        <h1 className="mt-3 text-5xl">المدينة نامت</h1>
        <p className="mt-4 text-muted-foreground">المافيا والطبيب والمحقق ياخدوا قرارهم بسرّية.</p>
        <Button className="mt-8" onClick={() => setPhase("day")}>
          طلّع النهار
        </Button>
      </section>
    );
  if (phase === "day")
    return (
      <section className="mafia-play mx-auto max-w-xl rounded-[2rem] p-8 text-center">
        <span className="eyebrow">مرحلة النهار</span>
        <h1 className="mt-3 text-4xl">المدينة صحيت</h1>
        <p className="mt-4 text-muted-foreground">
          في حدا انقتل الليلة؟ احكوا، حلّلوا، وانتبهوا مين مرتبك.
        </p>
        <div className="turn-timer mt-7">02:00</div>
        <Button className="mt-7" onClick={() => setPhase("vote")}>
          افتحوا التصويت
        </Button>
      </section>
    );
    ...
    return (
      <section className="mafia-play mx-auto max-w-xl rounded-[2rem] p-8 text-center">
        ...
        {!voteRevealed ? (
          <>
            <p className="mt-3 text-muted-foreground">
              خذ الجهاز، واختر بصمت. التصويت ما بيظهر للقعدة.
            </p>
            <Button className="mt-6" onClick={() => setVoteRevealed(true)}>
              أنا لحالي · افتح التصويت
            </Button>
  ...
  const eliminated = Number(Object.entries(tally).sort((a, b) => b[1] - a[1])[0]?.[0] ?? -1);
  return (
    <section className="mafia-play mx-auto max-w-xl rounded-[2rem] p-8 text-center">
      <Crown className="mx-auto h-10 w-10 text-gold" />
      <p className="eyebrow mt-3">نتيجة التصويت</p>
      <h1 className="mt-3 text-4xl">خرج {names[eliminated] ?? "لا أحد"}</h1>
      <p className="mt-4 text-muted-foreground">
        اكشفوا دوره الآن وكملوا ليلة جديدة إذا ما انتهت اللعبة.
      </p>
      <Button asChild className="mt-7">
        <Link to="/games">لعبة جديدة</Link>
      </Button>
