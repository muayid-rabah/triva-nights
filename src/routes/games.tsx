  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="heritage-page-shell">
        <section className="mx-auto max-w-6xl px-4 pb-8 pt-8 text-center sm:pt-14">
          <p className="eyebrow">اختاروا لعبتكم لليلة</p>
          <h1 className="mt-3 text-4xl sm:text-6xl font-black">الألعاب</h1>
          <p className="mx-auto mt-3 max-w-2xl text-base sm:text-lg text-muted-foreground">
            من نفس اللمّة، بأجواء مختلفة. اختاروا اللعبة وخلوّها تبلّش.
          </p>
        </section>

        <section className="mx-auto grid max-w-7xl grid-cols-1 gap-5 px-4 pb-16 sm:grid-cols-2 lg:grid-cols-3">
          {ARCADE_GAMES.map((game) => {
            const Icon = GAME_ICONS[game.slug];
            return (
              <article key={game.slug} className={`arcade-card arcade-${game.accent}`}>
                <div className="arcade-card-icon" aria-hidden="true">
                  <Icon />
                </div>
                <div className="relative z-10 flex h-full flex-col items-start">
                  ...
                  <Button asChild className="mt-7">
                    <Link to="/arcade" search={{ game: game.slug }}>
                      العب الآن
                    </Link>
                  </Button>
                </div>
