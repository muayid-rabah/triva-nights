...
function makePairs(): Pair[] {
  ...
  return shuffle(FORMATION).map((role) => {
    const choices = shuffle(BILLION_AUCTION_PLAYERS.filter((player) => player.role === role && !used.has(player.id)));
    const [publicPlayer, hiddenPlayer] = choices;
...
function labelRole(role: Exclude<BillionRole, "COACH">) {
  ...
  return <article className={`billion-v2-pitch side-${side}`}>
    ...
    <div className="billion-v2-field">
      {FORMATION.map((role, index) => {
        const player = byRole(role)[role === "DEF" || role === "MID" || role === "ATT" ? index - (role === "DEF" ? 1 : role === "MID" ? 3 : 5) : 0];
...
function simulate(teams: [string, string], squads: [BillionAuctionPlayer[], BillionAuctionPlayer[]]) {
  ...
  const aStrength = strength(squads[0]); const bStrength = strength(squads[1]);
  ...
  const attack = (team: TeamIndex) => squads[team].filter((player) => player.role === "ATT");
  const midfield = (team: TeamIndex) => squads[team].filter((player) => player.role === "MID");
  const keeper = (team: TeamIndex) => squads[team].find((player) => player.role === "GK");
  ...
  ([0, 1] as TeamIndex[]).forEach((team) => {
    for (let index = 0; index < score[team]; index += 1) {
      const scorer = attack(team)[index % attack(team).length]; const assister = midfield(team)[index % midfield(team).length] ?? scorer;
      events.push({ minute: minutes[goalIndex++] ?? 88, team, goal: true, text: `${assister.name} يرسل تمريرة دقيقة، و${scorer.name} ينهيها في الشباك!` });
    }
  });
  const savedBy = keeper(score[0] > score[1] ? 0 : 1);
  if (savedBy) events.push({ minute: 84, team: score[0] > score[1] ? 0 : 1, text: `${savedBy.name} يتدخل بتصدٍ حاسم في الدقائق الأخيرة.` });
...
export function BillionAuctionGame({ teams, budget = 200 }: { teams: [string, string]; budget?: number }) {
  ...
  function resolve(winner: TeamIndex, paid: number) {
    ...
    setResolution({ winner, loser, paid }); setError("");
  }
  ...
  if (!pair) return null;
  return <section className="billion-v2-game"><header className="billion-v2-title"><span className="eyebrow">مزاد المليار · الجولة {round + 1}/7 · مركز {labelRole(pair.role)}</span><p>البطاقة المخفية لن تظهر إلا بعد انتهاء المزايدة، وهي من نفس المركز دائماً.</p></header><div className="billion-v2-layout"><MiniPitch name={teams[0]} budget={budgets[0]} squad={squads[0]} side={0} /><div className="billion-v2-desk">{resolution ? <><div className="billion-v2-reveal"><div>{card({ player: pair.publicPlayer })}<small>{teams[resolution.winner]} دفع {resolution.paid}M</small></div><div>{card({ player: pair.hiddenPlayer })}<small>{teams[resolution.loser]} أخذها مجاناً</small></div></div><Button onClick={nextRound}>{round === 6 ? "حلّل المباراة" : "الجولة التالية"} <Play /></Button></> : <><span className="billion-v2-role">{labelRole(pair.role)}</span>{card({ player: pair.publicPlayer })}<div className="billion-v2-hidden-note">{card({ hidden: true })}</div><div className="billion-v2-bid"><strong>{bid || minimum}M</strong><small>{lastBidder === null ? `البداية من ${minimum}M` : `آخر مزايدة من ${teams[lastBidder]}`}</small></div><p>الدور على <b>{teams[bidder]}</b></p><div className="auction-bid-controls"><Button disabled={budgets[bidder] < Math.max(minimum, bid + 5)} onClick={() => bidNow(Math.max(minimum, bid + 5))}><Gavel /> +٥M</Button><Button disabled={budgets[bidder] < Math.max(minimum, bid + 10)} onClick={() => bidNow(Math.max(minimum, bid + 10))}>+١٠M</Button><Input type="number" min={Math.max(minimum, bid + 1)} max={budgets[bidder]} value={customBid} onChange={(event) => setCustomBid(event.target.value)} placeholder="اكتب مزايدتك" /><Button variant="outline" onClick={() => bidNow(Number(customBid))}>زايد</Button><Button variant="outline" onClick={pass}>أمرّر</Button></div>{error && <p className="billion-v2-error">{error}</p>}</>}</div><MiniPitch name={teams[1]} budget={budgets[1]} squad={squads[1]} side={1} /></div></section>;
}
